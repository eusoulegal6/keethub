...
-- Fix ambiguous player_id references and set_balderdash_ready no-op bug.
...
BEGIN
  ...
  SELECT pr.username, COALESCE(pr.avatar_config, '{}'::jsonb)
...
BEGIN
  ...
  SELECT COUNT(*), COUNT(*) FILTER (WHERE bp.is_ready)
...
BEGIN
  ...
  SELECT bp.id INTO actor_player_id
  FROM public.balderdash_players bp
  WHERE bp.room_id = p_room_id
    AND bp.user_id = current_user_id
    AND bp.is_connected = true;
  ...
  IF actor_player_id IS NULL OR actor_player_id <> room_row.selector_player_id THEN
    RETURN jsonb_build_object('success', false, 'error', 'Only the selector can choose the deck');
  END IF;

  SELECT * INTO prompt_row
  FROM public.balderdash_prompts bpr
  WHERE bpr.deck = selected_deck
    AND NOT (bpr.id = ANY(room_row.used_prompt_ids))
  ORDER BY random()
  LIMIT 1;
...
BEGIN
  ...
  SELECT * INTO prompt_row
  FROM public.balderdash_prompts bpr
  WHERE bpr.id = room_row.current_prompt_id;
  ...
  VALUES (p_room_id, room_row.round_number, actor_player_id, clean_answer)
  ...
  IF submitted_count >= active_count THEN
    ...
    WITH ordered AS (
      SELECT bo.id AS opt_id, row_number() OVER (ORDER BY random()) AS rn
...
BEGIN
  ...
  IF option_row.player_id = actor_player_id THEN
  ...
  VALUES (p_room_id, room_row.round_number, actor_player_id, p_option_id)
  ...
  IF vote_count >= active_count THEN
    ...
    FROM (
      SELECT bo.player_id AS author_id, COUNT(*)::int AS fooled_count
      FROM public.balderdash_votes bv
      JOIN public.balderdash_options bo ON bo.id = bv.option_id
      WHERE bv.room_id = p_room_id
        AND bv.round_number = room_row.round_number
...
BEGIN
  ...
  reveal_results := room_row.phase IN ('round_results', 'finished');

  -- Round-point computation via CTEs to avoid deeply nested correlated subqueries.
  WITH correct_guessers AS (
    SELECT bv.player_id AS guesser_id
    FROM public.balderdash_votes bv
    JOIN public.balderdash_options bo ON bo.id = bv.option_id
    WHERE bv.room_id = p_room_id
      AND bv.round_number = room_row.round_number
  ...
  round_points AS (
    SELECT
      bp.id AS pid,
      CASE WHEN reveal_results THEN
        (CASE WHEN cg.guesser_id IS NOT NULL THEN 1 ELSE 0 END)
        + COALESCE(fc.fooled_count, 0)
      ELSE 0 END AS points
    FROM public.balderdash_players bp
    LEFT JOIN correct_guessers cg ON cg.guesser_id = bp.id
    LEFT JOIN fooled_counts fc ON fc.author_id = bp.id
    WHERE bp.room_id = p_room_id
  )
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'roundPoints', rp.points
    ) ORDER BY bp.joined_at
  ), '[]'::jsonb) INTO players_json
  FROM public.balderdash_players bp
  JOIN round_points rp ON rp.pid = bp.id
  WHERE bp.room_id = p_room_id;
  ...
  IF room_row.phase IN ('voting', 'round_results', 'finished') THEN
    SELECT COALESCE(jsonb_agg(
      jsonb_build_object(
        'id', bo.id,
        'answer', bo.answer,
        'isCorrect', CASE WHEN reveal_results THEN bo.is_correct ELSE NULL END,
        'authorPlayerId', CASE WHEN reveal_results THEN bo.player_id ELSE NULL END,
        'authorName', CASE WHEN reveal_results THEN author.name ELSE NULL END,
        'votes', CASE WHEN reveal_results THEN (
          SELECT COALESCE(jsonb_agg(
            jsonb_build_object('playerId', voter.id, 'name', voter.name)
            ORDER BY voter.joined_at
          ), '[]'::jsonb)
          FROM public.balderdash_votes bv
          JOIN public.balderdash_players voter ON voter.id = bv.player_id
          WHERE bv.option_id = bo.id
        ) ELSE '[]'::jsonb END
      ) ORDER BY bo.sort_order
    ), '[]'::jsonb) INTO options_json
    FROM public.balderdash_options bo
    LEFT JOIN public.balderdash_players author ON author.id = bo.player_id
    WHERE bo.room_id = p_room_id
      AND bo.round_number = room_row.round_number;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'room', jsonb_build_object(
      ...
      'correctAnswer', CASE WHEN reveal_results THEN prompt_row.correct_answer ELSE NULL END
    ),
    ...
    'self', jsonb_build_object(
      'submitted', EXISTS (
        SELECT 1 FROM public.balderdash_submissions bs
        WHERE bs.room_id = p_room_id
          AND bs.round_number = room_row.round_number
          AND bs.player_id = self_player_id
      ),
